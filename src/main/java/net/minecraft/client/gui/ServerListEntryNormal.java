...
import com.anchorclient.client.feature.servers.impl.featured.Hypixel;
...
public class ServerListEntryNormal implements GuiListExtended.IGuiListEntry
{
    ...
    public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected)
    {
        ...
        if(server instanceof Server) {
            if(((Server) server).getType() == ServerType.PARTNER) {
                DrawingUtil.drawImg("Anchor/icon.png", x - 36, y, 32, 32);
            }

            if(((Server) server).getType() == ServerType.FEATURED) {
                if(server instanceof Hypixel) {
                    DrawingUtil.drawImg("Anchor/images/popular.png", x - 36, y, 32, 32);
                } else {
                    DrawingUtil.drawImg("Anchor/images/star.png", x - 36, y, 32, 32);
                }
            }
        }
